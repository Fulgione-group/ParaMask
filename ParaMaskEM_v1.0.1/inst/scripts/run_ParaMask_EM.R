...
while (i <= length(args)) {
  ...
  } else if (args[i] %in% c("--nSNPs", "-ns")) {
    nSNPs <- as.numeric(args[i + 1]); i <- i + 1
...
em_input_data <- ParaMaskEM::prepare_em_input(het, subsample = 10000, nSNPs = nSNPs, verbose = verbose)
...
#plot last iteration
tryCatch({
  ParaMaskEM::plot_EM_it(het = het, fit = EM_results$fit, weight1 = EM_results$weight1, rs = em_input_data$rs, rf = em_input_data$rf, iteration = EM_results$iteration, outpath = outpath, ID = ID)
}, error = function(e) message("Final EM plot failed: ", conditionMessage(e)))

#plot log likelihood convertion
tryCatch({
...
#classify SNPs based on log likeihood ratio
het <- ParaMaskEM::classify_snps_by_llr(het = het, probs1 = EM_results$probs1, probs2 = EM_results$probs2, regData3 = em_input_data$regData3 ,fit = EM_results$fit, cutoff = 0.99)

#plot classification
tryCatch({
  ParaMaskEM::plot_llr_classification(het = het, rs = em_input_data$rs, rf = em_input_data$rf, cutoff = 0.99, outpath = outpath, ID = ID)
...
#ensure non scientific numbers in position file
...
#plot distances
tryCatch({
  plot_distance_cutoff(distances = dist_cutoff_results$distances, dist_cutoff = dist_cutoff_results$dist_cutoff, p1 =  dist_cutoff_results$p1, p2 = dist_cutoff_results$p2, outpath = outpath, ID = ID)
}, error = function(e) {
