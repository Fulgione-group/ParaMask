...
if(boundfit){
  if(coef_fit3[4]> boundary){
    if(verbose){
      print(paste("taking a modified step. Upperbound exeeded, slope: ",as.character(coef_fit3[4]), ", with upper bound: ", as.character(boundary),". Using boundary as offset on minor allele frequency to refit...", sep = ""))
      tryCatch({
        fit3<-vglm(cbind(regData2$het, regData2$N - regData2$het) ~ Z + offset(MAF*boundary), data=regData2, family = betabinomial(lmu="clogloglink", lrho = "logitlink"),start=c(2,1,0,2),trace=TRUE, subset=(regData2$N > 1), weights = c(weight1,weight2))
      }, error = function(err){
        print(paste("ERROR in Mstep during VGAM fitting:  ",err))
        q(save = "no", status = 1, runLast = F)
      })
      coef_fit3<- coefficients(fit3)[1:3]
      coef_fit3[4]<-boundary
      offsetfit<-TRUE
    }else{
      tryCatch({
        suppressWarnings(fit3<-vglm(cbind(regData2$het, regData2$N - regData2$het) ~ Z + offset(MAF*boundary), data=regData2, family = betabinomial(lmu="clogloglink", lrho = "logitlink"),start=c(2,1,0,2),trace=TRUE, subset=(regData2$N > 1), weights = c(weight1,weight2)))
    }, error = function(err){
      print(paste("ERROR in Mstep during VGAM fitting:  ",err))
      q(save = "no", status = 1, runLast = F)
    })
...
for(iteration in 1:num_iterations){
  ...
  if(boundfit){
    if(coef_fit3_new[4]> boundary){
      if(verbose){
        print(paste("taking a modified step. Upperbound exeeded, slope: ",as.character(coef_fit3_new[4]), ", with upper bound: ", as.character(boundary),". Using boundary as offset on minor allele frequency to refit...", sep = ""))
        tryCatch({
          fit3<-vglm(cbind(regData2$het, regData2$N - regData2$het) ~ Z + offset(MAF*boundary), data=regData2, family = betabinomial(lmu="clogloglink", lrho = "logitlink"),start=c(2,1,0,2),trace=TRUE, subset=(regData2$N > 1), weights = c(weight1,weight2))
        }, error = function(err){
          print(paste("ERROR in Mstep during VGAM fitting:  ",err))
